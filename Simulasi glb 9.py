...
# Set layout wide agar simulasi lebih lebar
st.set_page_config(layout="wide")
st.title("Simulasi Gerak Lurus Beraturan (GLB) dengan Gedung")

st.markdown("""
**Deskripsi:** Simulasi ini menampilkan mobil bergerak lurus dengan kecepatan tetap di depan deretan gedung.  
...
#simulasi { position: relative; width: 100%; height: 300px; margin: auto; background: linear-gradient(to top, #a8d5e2 70%, #e0f7fa 30%); border: 2px solid #333; border-radius: 10px; overflow: hidden; }
...
<div id="simulasi">
  ...
  <script>
    ...
    const totalGedung=50; // banyak gedung agar penuh layar
...
function updateSimulasi(){
    ...
    chartKecepatan.update();
    if(posisi>=90) berhenti(); // limit 90 meter
}
